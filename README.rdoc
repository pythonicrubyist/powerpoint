...
'powerpoint' gem can generate a powerpoint presentaion based on a standard template:
    ...
    # Contents must be an array of strings that will be displayed as bullet items.
    title = 'Why Mac?'
    content = ['Its cool!', 'Its light.']
    @deck.add_textual_slide title, content

    # Creating an image Slie.
...
'powerpoint' gem has been tested using LibreOffice(4.2.1.1) and Apache OpenOffice(4.0.1) on Mac OS X Mavericks and Microsoft PowerPoint 2010 on Windows 7 and Google Docs(latest version as of March 2014).
